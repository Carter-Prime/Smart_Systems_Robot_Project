...
// checks if it can turn righ. obsolete in current version of maze
bool checkRight(struct sensors_ *ref){
...
//in an intersection go forward
void mazeForward(void){
    motor_forward(100,250);
    motor_forward(0,0);
}

//in an intersection turn left
void mazeLeft(struct sensors_ *ref, position *pos){
...
//in intersection turn right
void mazeRight(struct sensors_ *ref, position *pos){
...
// move to start position of maze which is at right most position
void mazeStartPosition(struct sensors_ *ref, position *pos){
    ...
    while (i < 4){
        reflectance_read(ref);
        if ((ref->l3 > 23000 || ref->r3 > 23000) && ref->l1 > 23000 && onLine){			// intersection check
            onLine = 0;
            
            if(pos->dir < 0)			// sets coords depending on the direction the robot came from to the intersection
                pos->x--;
            else if (pos->dir > 0)
                pos->x++;
            
            print_mqtt("Zumo009/tst","X:%d Y:%d DIR:%d", pos->x,pos->y,pos->dir); // debug to mqtt
            
            motor_forward(100,75); // move forward for a bit so the sensor readings are correct
            
            if(i == 0)						// else if to turn right on the first intersection and left on the last one
               mazeRight(ref, pos);
...
void zmain(void)
{
   ...
   mazeStartPosition(&ref,&pos); //go to start pos
    
   
    
    while(pos.y < 11){
        reflectance_read(&ref);
        if ((ref.l3 > 23000 || ref.r3 > 23000) && ref.l1 > 23000 && onLine){ // check for intersection
            
            onLine = 0;					// onLine set to 0 since robot is not following the line rn
            motor_forward(100,75);
            
            if(pos.dir == 0)			//update position
                pos.y++;
            else if(pos.dir < 0)
                pos.x--;
            else
                pos.x++;
            print_mqtt("Zumo009/tst","X:%d Y:%d DIR:%d", pos.x,pos.y,pos.dir);
            
            
            if(Ultra_GetDistance() < 15){	// check for obstacle
                forceturn = 3;
            }
            
            if (forceturn > 0){				// forceturn controls next 3 actions the robot does. Turn left straight right 
                if (forceturn == 3)			// first go left
                    mazeLeft(&ref,&pos);
                    
                else if(forceturn == 1){	// turns right backs up to intersection so it can check for obstacles again
                    mazeRight(&ref,&pos);
                    pos.y--;				// remove one y coord since we are backing up not going forward
                    motor_backward(0,1);
                    
                    while(1){				// backwards alg
                        if((ref.l3 > 23000 || ref.r3 > 23000) && ref.l1 > 23000)
                            break;
                ...
                else					// just go forward on second iteration
                    mazeForward();
                ...
                mazeForward();		// if there are no obstacles go forward
                
        }
        else{						// simple line following
            if(ref.r1 > 23000){
                motor_turn(100,0,1);
                onLine = 1;
            }
    ...
    //////////////////////////////////////////////////
    motor_backward(0,1);	// this is the last line. The algorithm checks its position and moves to the center 
							// of the maze and then goes traight to leave the maze
    ...
    if(pos.x > 0){					// move to middle from the right
        reflectance_read(&ref);
        mazeLeft(&ref,&pos);
        
        while(pos.x != 0){
            reflectance_read(&ref);
            if ((ref.l3 > 23000 || ref.r3 > 23000) && ref.l1 > 23000 && onLine){
                onLine = 0;
                pos.x--;
                print_mqtt("Zumo009/tst","X:%d Y:%d DIR:%d", pos.x,pos.y,pos.dir);
                mazeForward();
    ...
    else if(pos.x < 0){					// move to middle from the left
        reflectance_read(&ref);
        mazeRight(&ref,&pos);
    ...
    motor_forward(255, 300);			// random shit at the end
    spin(250,3000,1);
...
#if 0
//LINE FOLLOW FINAL dumb af solution
...
#if 0
// VALUE CHECK  program to check for sensor values with mqtt
