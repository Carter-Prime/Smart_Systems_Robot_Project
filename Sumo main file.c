...
//wait for user to press button function
...
//follow straight line then stop on black

void line_stop(struct sensors_ dig)
{
...
//Sumo challenge function

void sumo(struct sensors_ dig)
{
    motor_f(100, 500);
    while(dig.l1 == 0 || dig.l2 == 0 || dig.l3 == 0 || dig.r1 == 0 || dig.r2 == 0 || dig.r3 == 0)
    {
...
//sensor calibration for digital use
   ...
   int distance;
    ...
    while(true)
    {
        ...
        if (distance <= 10)
        {
            motor_forward(0,0);
            Beep(100, 150);
            motor_backward_left(50, 100, 2000);
        }
    }
...
void zmain(void)
{
    ...
    sensorCalibration(threshold_dig, 2);
    reflectance_set_threshold(threshold_dig[0], threshold_dig[1], threshold_dig[2], threshold_dig[3], threshold_dig[4], threshold_dig[5]);
    IR_Start();
    motor_start();
    line_stop(dig);
    IR_Start();
...
void zmain(void)
{
    ...
    vTaskDelay(3000);
    reflectance_read(&ref);
    while(ref.r1 + ref.r2 + ref.r3 + ref.l1 + ref.l2 + ref.l3 < 70000)
        {
            reflectance_read(&ref);
            motor_forward(100,0);
        }
        ...
        while(ref.r1 + ref.r2 + ref.r3 + ref.l1 + ref.l2 + ref.l3 < 70000)
        {
            ...
            if(ref.r1 + ref.r2 >20000 && ref.l1 < 10000)
            {
                motor_turn(75, 150, 10);
            }
            else if(ref.l1 + ref.l2 > 20000 && ref.r1 < 10000)
            {
                motor_turn(150, 75, 10);
            }
        }
...
void zmain(void)
{
    ...
    while (1)
    {
        ...
        if(random_number == 0)
        {
            motor_turn(160, 100, 10);
        }
        else if(random_number == 9)
        {
             motor_turn(100, 160, 10);
        }
        else if(random_number == 5)
        {
            motor_turn(100, 100, 10);
        }
